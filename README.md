...
[![tests](https://img.shields.io/badge/tests-100-0f766e)](#testing)
...
- **Dynamic pages** — a `MutationObserver` processes SPA and infinite-scroll
  content, including text a framework replaces inside an existing paragraph.
...
| Gate | Result |
|---|---|
| `bun test` | 100 tests across 11 files |
| `bun run typecheck` | clean (strict TypeScript) |
| `bun run build` + `verify` | both targets, all referenced files present |
| `web-ext lint` | 0 errors, 0 warnings, 0 notices |
| `bun run e2e` | 6/6 checks in real Chrome |
